Shader "MK/MKGlow/Standard (Specular setup)"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			#pragma fragment fragMKForwardBase

			#include "UnityStandardCoreMKGlowSystem.cginc"
			sampler2D _MKGlowTex;
			half _MKGlowTexStrength;
			fixed4 _MKGlowTexColor;
			float4 _MKGlowTex_ST;
			
			half4 fragMKForwardBase (VertexOutputForwardBase i) : SV_Target
			{
				fixed4 d = tex2D(_MKGlowTex, i.uv_MKGlowTex.xy * _MKGlowTex_ST.xy + _MKGlowTex_ST.zw) * _MKGlowTexColor;
				FRAGMENT_SETUP(s)
		...
		Pass
		{
			...
			#pragma fragment fragMKForwardAdd
			...
			half4 fragMKForwardAdd (VertexOutputForwardAdd i) : SV_Target
			{
				fixed4 d = tex2D(_MKGlowTex, i.uv_MKGlowTex.xy * _MKGlowTex_ST.xy + _MKGlowTex_ST.zw) * _MKGlowTexColor;
		...
		Pass
		{
			...
			#pragma fragment fragMKDeferred
			...
			)
			{
				...
				#endif
				s.diffColor += (d * _MKGlowTexStrength);
	...
	FallBack "VertexLit"
	CustomEditor "MK_StandardShaderGUI"
}
